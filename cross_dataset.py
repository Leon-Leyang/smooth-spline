...
class ModifiedModel(nn.Module):
    ...
    """
    def __init__(self, base_model, topk_layer, num_classes):
    ...
    def register_hooks(self):
        ...
        # Get all layers except the last one (assuming the last one is the classification layer)
        layers = list(self.base_model.children())[:-1]
        layers_to_hook = layers[-self.topk_layer:]
    ...
    def forward(self, x):
        ...
        if self.fc is not None:
            ...
            return features  # Return features if fc is not yet set
...
def transfer_linear_probe(model, pretrained_ds, transfer_ds, topk_layer=1, C=1):
    """
    Transfer learning.
    """
    ...
    modified_model = ModifiedModel(model, topk_layer, num_classes).to(device)
    ...
    # Determine input_features size from extracted features
...
def lp_then_replace_test_acc(beta_vals, pretrained_ds, transfer_ds, topk_layer=1, C=1):
    """
    ...
    model = transfer_linear_probe(model, pretrained_ds, transfer_ds, topk_layer, C)
...
def lp_then_replace_test_robustness(threat, beta_vals, pretrained_ds, transfer_ds):
    ...
    """
    model = get_pretrained_model(pretrained_ds)
    model = transfer_linear_probe(model, pretrained_ds, transfer_ds, topk_layer=1, C=1)
    replace_and_test_robustness(model, threat, beta_vals, f'{pretrained_ds}_to_{transfer_ds}')


def replace_then_lp_test_acc(beta_vals, pretrained_ds, transfer_ds, topk_layer=1, C=1):
    """
    ...
    transfer_model = transfer_linear_probe(orig_model, pretrained_ds, transfer_ds, topk_layer, C)
...
def get_args():
    ...
    parser.add_argument('--topk_layer', type=int, default=1, help='Number of top layers to extract features from')
...
if __name__ == '__main__':
    ...
    set_logger(name=f'{f_name}_{args.order}_topk{args.topk_layer}_C{args.C}_seed{args.seed}')
        ...
        for transfer_ds in transfer_datasets:
            ...
            if pretrained_ds == transfer_ds:    # Test on the same dataset
                ...
                if args.order == 'lp_replace':
                    lp_then_replace_test_acc(betas, pretrained_ds, transfer_ds, args.topk_layer, args.C)
                elif args.order == 'replace_lp':
                    replace_then_lp_test_acc(betas, pretrained_ds, transfer_ds, args.topk_layer, args.C)
                else:
