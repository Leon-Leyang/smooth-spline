        ...
        for line in file:
            if robustness:
                ...
                if match:
                    ...
                    # if any(ds in current_dataset for ds in ["med_mnist", "celeb_a"]):
                    #     continue
...
def compute_statistics(log_files, robustness=False):
    ...
    for dataset in stats_new_accuracies:
        if stats_baseline_accuracies[dataset][0] > 0:
            ...
            if new_mean > baseline_mean:
                valid_betas.append(stats_betas[dataset][0])
    ...
    overall_normalized_improvement = sum(normalized_improvements.values()) / len(normalized_improvements) if normalized_improvements else 0
    overall_absolute_improvement = sum(absolute_improvements.values()) / len(absolute_improvements) if absolute_improvements else 0
    overall_average_beta = sum(valid_betas) / len(valid_betas) if valid_betas else 0

    return stats_new_accuracies, stats_baseline_accuracies, stats_betas, normalized_improvements, absolute_improvements, overall_normalized_improvement, overall_absolute_improvement, overall_average_beta

if __name__ == "__main__":
    log_files = [
        "./logs/post_replace_classification_replace_lp_coeff0.5_topk1_reg1_resnet152_more_ds_seed42.log",
        "./logs/post_replace_classification_replace_lp_coeff0.5_topk1_reg1_resnet152_more_ds_seed43.log",
        "./logs/post_replace_classification_replace_lp_coeff0.5_topk1_reg1_resnet152_more_ds_seed44.log"
    ]
    ...
    stats_new_accuracies, stats_baseline_accuracies, stats_betas, normalized_improvements, absolute_improvements, overall_normalized_improvement, overall_absolute_improvement, overall_average_beta = compute_statistics(
    ...
    print(f"Overall Average Best Beta Across All Datasets: {overall_average_beta:.2f}")
